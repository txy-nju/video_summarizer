...
def fusion_drafter_node(state: VideoSummaryState, llm_model: BaseModel | None = None) -> dict:
    ...
    system_prompt = (
        ...
        "3. 📝 专业排版规范：输出必须使用易于阅读的 Markdown 语法。"
        "建议包含：【内容导读】、【核心内容解析】、【关键结论】、【总结与建议】等模块。\n"
        "4. ⏰ 时间戳锚定：每个实质性陈述句末须附 [HH:MM] 时间戳引用"
        "（直接来自输入证据中的时间戳字段），以供读者精准定位视频片段。\n"
        "5. 🚫 禁止开场白：严禁输出\"好的，作为...\"、\"我将严格遵循...\"等确认性开场白，"
        "直接从正文开始输出。\n"
        "6. #️⃣ 标题要求：必须使用 # 一级标题为报告命名，"
        "标题须基于视频核心内容提炼一个有意义的名称，"
        "严禁使用\"视频总结报告\"之类的泛化标题。"
    )
