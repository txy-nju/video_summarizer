...
> ⚠️ **核心原则**：LLM 不擅长真正的随机选择——当看到简历写了"灰度直方图阈值 0.90"时，自然会想问场景抽帧；看到"wave_dispatch + synthesis_barrier"就想问并行调度细节。这种注意力偏好会导致每次面试问同样的题。以下流程通过**可见的掷骰 + 显式编号选题**来强制打破这种偏好。
...
- ❌ 看到"wave_dispatch + synthesis_barrier"就问并行调度细节——除非骰子恰好选到这道题
- ❌ 看到"SELF_RAG_MAX_REVISIONS 上限"就问 Self-RAG 循环机制——除非骰子恰好选到这道题
- ❌ 每次都从简历中最显眼的关键词出发提问
